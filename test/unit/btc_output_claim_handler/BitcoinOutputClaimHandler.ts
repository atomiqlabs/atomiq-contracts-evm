...
import { BitcoindBlockheader, BitcoindTransaction, getBlockWithTransactions, getTransaction, randomBitcoinHeight } from "../../utils/bitcoin_rpc_utils";
...
function getTxoHash(value: bigint, outputScript: string) {
    return hre.ethers.keccak256(
        "0x"+toBuffer(value, 8, "be").toString("hex")+hre.ethers.keccak256("0x"+outputScript).substring(2)
    ).substring(2);
}
...
describe("BitcoinOutputClaimHandler", function () {
    async function deploy() {
        const BtcRelay = await hre.ethers.getContractFactory("BtcRelay");
        const BitcoinOutputClaimHandler = await hre.ethers.getContractFactory("BitcoinOutputClaimHandler");
        const contract = await BitcoinOutputClaimHandler.deploy();
    ...
    it("Valid random witness", async function () {
        ...
        for(let i=0;i<10;i++) {
            ...
            const txoHash = getTxoHash(BigInt(output.value), output.script.toString("hex"));
    ...
    it("Valid real witness", async function () {
        ...
        for(let i=0;i<10;i++) {
            ...
            const txoHash = getTxoHash(BigInt(Math.floor((output.value * 100_000_000) + 0.5)), output.scriptPubKey.hex);
    ...
    it("Invalid empty witness", async function () {
        const {contract, BtcRelay} = await loadFixture(deploy);
        await expect(contract.claim(randomBytes32(), "0x")).to.be.revertedWith("btcoutlock: witness length");
    });
    ...
    it("Invalid incorrect commitment witness", async function () {
        ...
        )).to.be.revertedWith("btcoutlock: invalid commitment");
    });
    ...
    it("Invalid block confirmations", async function () {
        ...
        await expect(contract.claim(
            getCommitmentHash(txoHash, confirmations, await relayContract.getAddress()),
            getWitness(txoHash, confirmations, await relayContract.getAddress(), genesis, vout, tx.toHex(), position, proof)
        )).to.be.revertedWith("btcoutlock: confirmations");
    });
    ...
    it("Invalid txoHash doesn't match (due to wrong outputAmount)", async function () {
        ...
        const vout = Math.floor(tx.outs.length*Math.random());
        const output = tx.outs[vout];
        const fakeTxoHash = getTxoHash(BigInt(output.value), output.script.toString("hex"));
        const expectedTxoHash = getTxoHash(BigInt(output.value + 1), output.script.toString("hex"));
        ...
        await expect(contract.claim(
            getCommitmentHash(expectedTxoHash, confirmations, await relayContract.getAddress()),
            getWitness(expectedTxoHash, confirmations, await relayContract.getAddress(), genesis, vout, tx.toHex(), position, proof)
        )).to.be.revertedWith("btcoutlock: Invalid output");
    });
    ...
    it("Invalid txoHash doesn't match (due to wrong outputScript)", async function () {
        ...
        const output = tx.outs[vout];
        const fakeTxoHash = getTxoHash(BigInt(output.value), output.script.toString("hex"));
        const expectedTxoHash = getTxoHash(BigInt(output.value), output.script.toString("hex")+"deadbeef");
