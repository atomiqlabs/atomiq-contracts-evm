...
library Events {
    ...
    event Claimed(bytes32 indexed ownerAndVaultId, address indexed recipient, bytes32 indexed btcTxHash, address caller, bytes32 executionHash, address frontingAddress, uint32 withdrawCount, uint64 amount0, uint64 amount1);
    event Fronted(bytes32 indexed ownerAndVaultId, address indexed recipient, bytes32 indexed btcTxHash, address caller, bytes32 executionHash, uint64 amount0, uint64 amount1);
}
